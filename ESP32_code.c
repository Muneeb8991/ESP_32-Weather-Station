...
//Specifying Network SSID and password 
...
//Initalise DHT22 sensor 
 dht.begin(); 
  ...
  //Wait 5 minutes for the sensor to acclimate   
  delay(30000); 
...
//In loop Function refresh web page every second and send to sheets every 30 min 
 void loop() {   
  int i=0;   
  while(i<1790){     
   server.handleClient();    
...
{ 
  ...
  String temp;   temp  = "temperature="+String(t, 6);   
 temp += "&humidity="+String(h, 6);   
 temp += "&heatindex="+String(hic,6);   
 send_to_sheet(temp); 
...
//Function to send data to sheets void send_to_sheet(String params) { 
   ...
   //Recieve response from Web App    
   int httpCode = http.GET();      
   Serial.print("HTTP Status Code: "); 
   Serial.println(httpCode);    String payload;    
   if (httpCode > 0) { 
...
//Function for when HTTP response code is 200 or Ok 
void handle_OnConnect() {   
 float  t=dht.readTemperature();   
 float  h=dht.readHumidity(); 
 float hic=dht.computeHeatIndex(t, h, false);   
 server.send(200, "text/html", SendHTML(t,h,hic)); } 
 
//Function for when HTTP response is 404 not found 
...
String SendHTML(float t,float h,float hic){   String ptr = "<!DOCTYPE html> <html>\n"; 
  ...
  ptr +="font-family: Arial;";   ptr +="display: inline-block;";   ptr +="margin: 0px auto;";   
  ptr +="text-align: center;}";   ptr +="</style>";   ptr +="<script>\n";   ptr +="setInterval(loadDoc,1000);\n";   
  ...
  ptr +=h;   ptr +="%</p>";   ptr +="<p>Heat Index: ";   ptr +=hic;   ptr +="&deg;C</p>";   
  ptr +="</div>\n";   ptr +="</body>\n";   ptr +="</html>\n"; 
  return ptr; 
} 
